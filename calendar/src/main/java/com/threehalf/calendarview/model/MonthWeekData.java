...
public class MonthWeekData {
    ...
    public MonthWeekData(int position) {
        ...
        if( CellConfig.currentDate==null){
            CellConfig.currentDate = new DateData(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, calendar.get(Calendar.DAY_OF_MONTH));
        }
        calendar.set(CellConfig.currentDate.getYear(), CellConfig.currentDate.getMonth()-1, CellConfig.currentDate.getDay());
    ...
    private void getPointDate() {
        ...
        Log.i("我是Month", "是我变了吗？" + CellConfig.w2mPointDate.getYear()+"==="+ CellConfig.w2mPointDate.getMonth());
        ...
        if (realPosition == CellConfig.middlePosition) {
            CellConfig.m2wPointDate = new DateData(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, calendar.get(Calendar.DAY_OF_MONTH));
            Log.v("我是Month", "哈哈当前页面锚点"+realPosition+"====================" + CellConfig.m2wPointDate.toString());
        } else {
        ...
        Log.e("getPointDate7777777", " 最终滚动后：" + (calendar.get(Calendar.MONTH) + 1) + "月" + calendar.get(Calendar.DAY_OF_MONTH));
...
//            weekIndex--;
    ...
    private void initMonthArray() {
        ...
        for (int preDay = lastMonthDayNumber - preNumber + 1; preDay < lastMonthDayNumber + 1; preDay++) {
            DateData dateData=new DateData(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, preDay);
            dateData.setDateType(0);
        ...
        for (int day = 1; day < thisMonthDayNumber + 1; day++) {
            DateData dateData= new DateData(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, day);
            dateData.setDateType(1);
        ...
        for (int afterDay = 1; afterDay < afterNumber; afterDay++) {
            DateData dateData= new DateData(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, afterDay);
            dateData.setDateType(2);
            addDate = new DayData(dateData);
            addDate.setTextColor(Color.LTGRAY);
            monthContent.add(addDate);
        }
